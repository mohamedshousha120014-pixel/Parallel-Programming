...
public class EtlController {
    ...
    public ResponseEntity<Map<String, Object>> getStats() {
        ...
        List<Map<String, Object>> departments = deptStats.stream().map(row -> {
            ...
            dept.put("avgSalary", String.format("$%.0f", (Double) row[2]));
            return dept;
        ...
        response.put("message", threads.size() > 1
                ? "✅ Multi-threading confirmed: " + threads.size() + " threads processed the data"
                : "⚠️ Only 1 thread used — try increasing chunk count or data size");
